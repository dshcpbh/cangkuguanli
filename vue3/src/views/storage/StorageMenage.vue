...
function handleDelete(row) {   // 删除用户
  proxy.$http.get('/storage/delete?id='+row.id).then(res => res.data).then(res => {
    if (res.code === 200) {
      ElMessage({
        message: '删除用户成功',
        type: 'success',
...
function doUpdate() {
  proxy.$http.post('/storage/update', form).then(res => res.data).then(res => {
    if (res.code === 200) {
      console.log(res)
...
function doAdd() {
  proxy.$http.post('/storage/save', form).then(res => res.data).then(res => {
    if (res.code === 200) {
      // console.log(res)
      centerDialogVisible.value = false
      ElMessage({
...
function getUserList() {
  loading.value = true
  proxy.$http.post('/storage/queryPage', {
